classdef FslmathsOptions < mlfsl.FslOptions
	%% FSLMATHSOPTIONS  
	...
	%  $Revision$
 	%  was created 13-Jan-2016 09:55:34
 	%  by jjlee,
	...
	properties
        ...
        bptf %   <hp_sigma> <lp_sigma> 
             % : (-t in ip.c) Bandpass temporal filtering; nonlinear highpass and Gaussian linear lowpass (with sigmas in volumes, not seconds); 
             %   set either sigma<0 to skip that filter
        roc % <AROC-thresh> <outfile> [4Dnoiseonly] <truth> 
            %  : take (normally binary) truth and test current image in ROC analysis against truth.
            %    <AROC-thresh> is usually 0.05 and is limit of Area-under-ROC measure FP axis. 
            %    <outfile> is a text file of the ROC curve (triplets of values: FP TP threshold). 
            %    If the truth image contains negative voxels these get excluded from all calculations. 
            %    If <AROC-thresh> is positive then the [4Dnoiseonly] option needs to be set, 
            %    and the FP rate is determined from this noise-only data, and is set to be the fraction 
            %    of timepoints where any FP (anywhere) is seen, as found in the noise-only 4d-dataset. 
            %    This is then controlling the FWE rate. If <AROC-thresh> is negative the FP rate is 
            %    calculated from the zero-value parts of the <truth> image, this time averaging voxelwise 
            %    FP rate over all timepoints. In both cases the TP rate is the average fraction of 
            %    truth=positive voxels correctly found.
 	end
	...
	methods 
        ...
        function this = set.roi(this, s)
            assert(ischar(s));
            this.roi = s;
        end
        function this = set.bptf(this, s)
            assert(ischar(s));
            this.bptf = s;
        end
        function this = set.roc(this, s)
            assert(ischar(s));
            this.roc = s;
        end
 	end 
